...
{{
  ...
  "preco": {price:.2f},
  "descricao": "descrição detalhada, 3-5 parágrafos, benefícios, garantia ML",
  "categoria": "{cat}",
  "condicao": "Novo",
  "tags_busca": ["tag1", "tag2", "tag3", "tag4", "tag5 com erro proposital"],
  "sku": "{(p.ml_code or 'ML')}-{datetime.now().strftime('%d%m%y')}",
  "texto_x1": "mensagem para enviar ao comprador explicando segurança ML, entrega rápida, garantia"
}}"""
    ...
    try:
        ...
        result = {
            ...
            "texto_x1":   f"Olá! Produto 100% novo, vendido pelo Mercado Livre. Se não chegar, o ML devolve. Link: {url}",
        }
...
async def generate_tiktok_script(
    ...
    prompt = f"""Você é especialista em vídeos virais no TikTok.

Produto: {title}
Preço: R$ {price:.2f}
Link: {url}

Script TikTok 15-22s. Responda SOMENTE com JSON válido:

{{
  "gancho_0_3s": "frase de abertura impactante. Max 15 palavras.",
  "problema_3_6s": "dor do espectador sem o produto. Max 15 palavras.",
  "solucao_6_12s": "produto como solução. Max 20 palavras.",
  "prova_12_18s": "benefício concreto. Max 15 palavras.",
  "cta_18_22s": "call to action terminando com 'Link na bio'. Max 10 palavras.",
  "legenda": "legenda TikTok até 200 chars com emojis",
  "hashtags": ["5", "hashtags", "relevantes"],
  "musica_sugerida": "estilo de música",
  "dica_visual": "o que mostrar na tela"
}}"""
    ...
    try:
        ...
        result = {
            ...
            "legenda":        f"🔥 {title} por R$ {price:.2f}! Link na bio 👆",
...
async def generate_youtube_script(
    ...
    prompt = f"""Você é especialista em YouTube de afiliados.

Produto: {title}
Preço: R$ {price:.2f}
Link: {url}

Roteiro + metadados YouTube. Responda SOMENTE com JSON válido:

{{
  "titulo_video": "título SEO até 70 chars",
  "roteiro": {{
    "intro_0_30s": "script introdução",
    "contexto_30_90s": "por que alguém precisa",
    "demonstracao_90_180s": "como funciona",
    "prova_social_180_240s": "avaliações e credibilidade",
    "comparativo_240_300s": "por que esse e não outro",
    "cta_300s": "call to action com link"
  }},
  "descricao_youtube": "descrição SEO até 400 chars com link afiliado",
  "tags": ["12", "tags", "youtube"],
  "timestamps": ["0:00 Introdução", "0:30 Por que usar", "1:30 Demo", "3:00 Vale a pena?"],
  "thumbnail_texto": "TEXTO THUMBNAIL CAPS 4 PALAVRAS",
  "keyword_principal": "keyword SEO principal"
}}"""
    ...
    try:
        ...
        result = {
            ...
            "descricao_youtube": f"Review completo: {title} por R$ {price:.2f}.\n🛒 {url}\n\n#review #mercadolivre",
